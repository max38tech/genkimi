...
admin.initializeApp();
exports.rakutenSearch = functions.https.onCall(async (data, context) => {
    var _a, _b, _c;
    const barcode = data.barcode;
    if (!barcode) {
        throw new functions.https.HttpsError("invalid-argument", "The function must be called with a 'barcode' argument.");
    }
    const appId = process.env.RAKUTEN_APP_ID;
    if (!appId) {
        console.error("RAKUTEN_APP_ID is not set in environment variables.");
        throw new functions.https.HttpsError("failed-precondition", "Backend configuration missing: Rakuten Application ID.");
    }
    try {
        ...
        throw new functions.https.HttpsError("internal", "An error occurred while fetching data from Rakuten Ichiba.");
    }
});
